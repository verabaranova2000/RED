...
def show_refined_rwfn_data(orbital_names, r_old, P_old_dict, Q_old_dict,
    ...
    Исходные данные:
        r, orbital_names, P(r), Q(r) —
            радиальные сетки и компоненты орбиталей,
            полученные из релятивистского расчёта GRASP
            (чтение с помощью функции read_rwfn_plot).

    Предобработанные данные:
        r_new, P_new(r), Q_new(r) —
            результат численной предобработки
            (сгущение радиальной сетки и асимптотическое продолжение),
            выполненной функцией refine_rwfn_data.
