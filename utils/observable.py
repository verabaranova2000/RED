...
class ObservableSettings:
    ...
    def _wrap_value(self, name, value):
        """
        Преобразует обычные Python-значения в реактивные контейнеры.

        list заменяется на ObservableList, dict — на ObservableDict.
        Вложенные ObservableSettings остаются без изменений.
        """        
        """
        Фабрика реактивных обёрток.
        Перехватывает обычные Python-значения и заменяет их на реактивные аналоги.

        Было:
          corrections = []
        Становится:
          ObservableList([], notify, path="corrections")

        ⚙️ 2. Когда он срабатывает
        ------
        Он вызывается здесь:
          __setattr__(name, value)
          👉 ДО того, как значение записано в объект
        ...
        ------
          ...
          __setattr__
                  ↓
          wrap_value("form", "Gaussian")
                  ↓
          (либо оставляем str)
                  ↓
          или превращаем list → ObservableList
                  ↓
          сохраняем в объект
        """
    ...
    def __setattr__(self, name, value):
        """"
        Вызывается каждый раз, когда я присваиваю атрибут.
        1. settings генерирует событие. Он говорит:
          - “изменилось поле form”
          - или “изменилось поле blackman.mode”

        __setattr__ — это pipeline:
            1. получили value
            2. трансформировали (wrap)
            3. сохранили
            4. уведомили
            Код это четко отражает:
              value = wrap(value)
              save(value)
              notify()
        """
        """
        Перехватывает присваивание атрибутов и отправляет уведомление об изменении.

        При установке пользовательских полей:
        - выполняется обёртка list/dict в реактивные контейнеры;
        - вложенные ObservableSettings получают рекурсивную привязку;
        - вызывается callback с полным путём изменённого поля.

        Приватные атрибуты, начинающиеся с "_", устанавливаются без уведомления.
        """        
